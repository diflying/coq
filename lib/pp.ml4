...
let open_tag name attrs = [< 'Ppcmd_open_tag (name, attrs) >]
let close_tag name = [< 'Ppcmd_close_tag name >]
...
let mode_quote strm =
  match !print_mode with
  | Print_emacs -> [< str emacs_quote_start; hov 0 strm; str emacs_quote_end >]
  | Print_pgip -> tag "errorresponse" [("fatality", "nonfatal")] (hov 0 strm)
  | Print_normal -> hov 0 strm

let warnbody strm = mode_quote (str "Warning: " ++ strm)
...
let msg_debug x = msgnl (mode_quote x)
