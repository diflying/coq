...
let default_pr_goal gs =
  ...
  in
    preamb ++
    str"  " ++ hv 0 (tag "hyps" [] penv ++ fnl () ++
                       str (emacs_str "")  ++
                       str "============================" ++ fnl ()  ++
                       thesis ++ str " " ++  tag "goal" [] pc) ++ fnl ()
...
let default_pr_subgoals close_cmd sigma seeds = function
  | [] ->
	...
	| None ->
	    let exl = Evarutil.non_instantiated sigma in
	      if exl = [] then
		tag "normalresponse" [("status", "no-more-subgoals")] (str"No more subgoals." ++ fnl ()
  ...
  | [g] ->
      ...
      tag "normalresponse" [("goal-id", Goal.uid g); ("subgoals", "1")] (v 0 (
  ...
  | g1::rest ->
      ...
      tag "normalresponse" [("goal-uid", Goal.uid g1); ("subgoals", string_of_int (List.length rest+1))] (v 0 (
